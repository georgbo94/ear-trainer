/* =========================================================
   app.js — final rewrite
   - Users: Guest + named, full CRUD
   - Menus: only consistent options visible (constraint solver)
   - Guess input: digits/commas/spaces only
   - Feedback restored
   ========================================================= */
...
/* -------------------------
   ...
   (function initApp() {
     ...
     window.addEventListener("keydown", (e) => {
       ...
       // If typing (input focused + not empty), let space act normally
       if (document.activeElement === el.guessInput) {
         if (el.guessInput.value.trim() !== "") return;
         // if empty, fall through and handle like replay
       }
     
       const cur = trainer.current;
       if (!cur) {
         // No trial → nothing to replay
         e.preventDefault();
         return;
       }
     
       if (!cur.answered) {
         // Unanswered trial → replay chord
         trainer.replay();
       } else {
         // Answered → toggle truth vs guess
         const last = trainer.log[trainer.log.length - 1];
         if (!last) return;
     
         if (!spaceToggle) {
           trainer.playGuess(last.rel);   // play truth
         } else {
           trainer.playGuess(last.guess); // play guess
         }
     ...
     function updateButtons() {
      ...
      if (!cur) {
        el.submitBtn.textContent = "Submit Guess";
     ...
     /* ---------- set/guess controls ---------- */
     el.newSetBtn.onclick = () => {
       spaceToggle = false;             // 🔄 reset comparison toggle
       trainer.nextTrial();
       updateButtons();
       el.feedback.innerHTML = "";
       try { el.guessInput.focus(); } catch {}
     };
   
     el.replaySetBtn.onclick = () => trainer.replay();
     ...
     el.submitBtn.onclick = () => {
      ...
      if (!cur.answered) {
        // First submission: grade the guess
        ...
        // Already answered: clicking again just replays the last guess
        const last = trainer.log[trainer.log.length - 1];
